...
class GifAnimation:
    """Класс для управления анимированными GIF"""

    def __init__(self, frames, durations):
        """
        frames: список pygame.Surface объектов (кадры)
        durations: список длительностей каждого кадра в миллисекундах
        """
        self.frames = frames
...
class AlternetExplorer:
    def __init__(self):
        ...
        # ACS-1: Дефолтные цвета контента (сохраняем для сброса)
        self.DEFAULT_TEXT_BG = (255, 255, 255)  # Белый фон для текста
        self.DEFAULT_TEXT_COLOR = (0, 0, 0)  # Черный текст
        self.DEFAULT_LINK_COLOR = (0, 0, 128)  # Темно-синие ссылки

        # ACS-1: Текущие цвета контента (могут быть переопределены страницей)
        self.TEXT_BG = self.DEFAULT_TEXT_BG
        self.TEXT_COLOR = self.DEFAULT_TEXT_COLOR
        self.LINK_COLOR = self.DEFAULT_LINK_COLOR
        ...
        # Кэш для изображений
        self.image_cache = {}
        # Хранилище GIF анимаций: {url: GifAnimation}
        self.gif_animations = {}
        # Флаг готовности стилей (предотвращает мерцание при загрузке ACS)
        ...
        # Инициализация
        self.update_layout(self.screen_width,
                           self.screen_height)  # ИСПРАВЛЕНО: инициализируем все rect перед первым рендером
    ...
    def draw_url_input(self):
        ...
        if self.url_input_active and self.url_input_visible:
            cursor_x = self.url_input_rect.x + 5 + min(self.fonts['url'].size(display_text[:self.url_input_cursor])[0],
                                                       self.url_input_rect.width - 10)
    ...
    def load_page(self, url):
        ...
        def load_thread():
            try:
                ...
                # ACS-1: Парсим стилевые директивы и применяем к странице
                print("[ACS] Начинаем парсинг ACS директив...")
                cleaned_markdown, acs_styles = self.parse_acs_styles(self.markdown_text)

                # Применяем извлеченные стили
                self.TEXT_BG = acs_styles['bg']
                self.TEXT_COLOR = acs_styles['text']
                self.LINK_COLOR = acs_styles['link']

                # Парсим очищенный от ACS директив markdown
                self.ast = self.parser.parse(cleaned_markdown)
                ...
                self.scroll_offset = 0
                self.max_scroll = 0
                self.styles_ready = True  # Стили применены, можно рендерить
                self.loading = False
                ...
                # При ошибке возвращаем дефолтные цвета
                self.TEXT_BG = self.DEFAULT_TEXT_BG
                self.TEXT_COLOR = self.DEFAULT_TEXT_COLOR
                self.LINK_COLOR = self.DEFAULT_LINK_COLOR
                self.styles_ready = True
    ...
    def parse_acs_styles(self, markdown_text):
        """Парсит ACS-1 директивы из первой строки документа

        Спецификация: <!-- ACS: bg=#XXXXXX; text=#XXXXXX; link=#XXXXXX -->
        Возвращает: (cleaned_text, styles_dict)
        """
        ...
        # Проверяем сигнатуру ACS
        acs_pattern = r'^<!--\s*ACS:\s*(.+?)\s*-->$'
        match = re.match(acs_pattern, first_line, re.IGNORECASE)

        if not match:
            # ACS директива не найдена - возвращаем дефолтные цвета
            return markdown_text, {
                'bg': self.DEFAULT_TEXT_BG,
                'text': self.DEFAULT_TEXT_COLOR,
                'link': self.DEFAULT_LINK_COLOR
            }

        # Извлекаем параметры из директивы
        ...
        # Парсим параметры (поддерживаем любой порядок и пробелы)
        param_pattern = r'(bg|text|link)\s*=\s*#([0-9A-Fa-f]{6})'
        for param_match in re.finditer(param_pattern, params_str):
            param_name = param_match.group(1).lower()
            hex_value = param_match.group(2).upper()

            # Конвертируем HEX в RGB tuple
            r = int(hex_value[0:2], 16)
            g = int(hex_value[2:4], 16)
            b = int(hex_value[4:6], 16)
            styles[param_name] = (r, g, b)

            print(f"[ACS] Найден параметр {param_name}: #{hex_value} -> RGB{styles[param_name]}")

        # Удаляем первую строку с директивой из контента
        cleaned_text = lines[1] if len(lines) > 1 else ''

        print(f"[ACS] Применены стили - bg: {styles['bg']}, text: {styles['text']}, link: {styles['link']}")
        return cleaned_text, styles
    ...
    def load_gif_animation(self, url, response_content):
        """Загружает анимированный GIF используя PIL и конвертирует в pygame формат"""
        try:
            # Открываем GIF через PIL
            pil_image = PILImage.open(BytesIO(response_content))
            ...
            while True:
                try:
                    ...
                    # Конвертируем в RGBA для pygame
                    ...
                    # Получаем длительность кадра (по умолчанию 100ms если не указана)
            ...
            # Если это статичный GIF (1 кадр), обрабатываем как обычное изображение
            if len(frames) == 1:
                print(f"[GIF] Статичный GIF, обработан как обычное изображение")
                return frames[0], False

            print(f"[GIF] Загружено {len(frames)} кадров, средняя длительность {sum(durations) / len(durations):.0f}ms")

            # Создаем объект анимации
            animation = GifAnimation(frames, durations)
            self.gif_animations[url] = animation

            return animation.get_current_frame(), True

        except Exception as e:
            print(f"[GIF] Ошибка загрузки анимации: {str(e)}")
            raise

    def load_image(self, url, base_url):
        """Универсальная загрузка изображений с поддержкой редиректов http→https и анимированных GIF"""
        ...
        for attempt_url in urls_to_try:
            try:
                ...
                is_gif = ext == '.gif' or 'image/gif' in content_type

                if is_gif:
                    print(f"[IMAGE] Обнаружен GIF, загружаем как анимацию...")
                    img, is_animated = self.load_gif_animation(full_url, response.content)

                    if is_animated:
                        print(f"[IMAGE] Анимированный GIF успешно загружен: {img.get_size()}")
                    else:
                        print(f"[IMAGE] Статичный GIF загружен: {img.get_size()}")
                else:
                    # Обычное изображение (PNG, JPG и т.д.)
    ...
    def handle_scroll(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 4:  # Колесо вверх
                ...
                self.scroll_offset = min(self.max_scroll - (self.screen_height - 100),
    ...
    def handle_url_input(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RETURN:
                ...
                char = event.unicode
                if char:
                    self.url_input = self.url_input[:self.url_input_cursor] + char + self.url_input[
                                                                                     self.url_input_cursor:]
    ...
    def run(self):
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                pygame.MOUSEBUTTONDOWN, pygame.MOUSEMOTION, pygame.MOUSEBUTTONUP, pygame.MOUSEWHEEL):
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                        if self.popup_active:
                            ...
                            if self.popup_type == 'bookmarks':
                                ...
                                if hasattr(self,
                                           'add_bookmark_button_rect') and self.add_bookmark_button_rect.collidepoint(
                                        event.pos):
